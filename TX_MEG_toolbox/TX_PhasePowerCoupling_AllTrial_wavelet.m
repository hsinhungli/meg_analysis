function [ chl_block_comodulation ] = TX_PhasePowerCoupling_AllTrial_wavelet( ft_cell,trl_num,channels,time_range)
...
Phase_data =  TX_Phase_wavelet( ft_cell,time_range,PhaseFreqVector,channels,trl_num );
Power_data =  TX_Power_wavelet( ft_cell,time_range,AmpFreqVector,channels,trl_num );
...
for chl = 1:length(channels)
    ...
    for trl = 1:trl_num
        ...
        for jj=1:length(PhaseFreqVector)
            ...
            PhaseFreqTransformed{trl}(jj, :) = squeeze(Phase_data{trl}(channels(chl),jj,:));
        end
    ...
    chl_block_comodulation{channels(chl)} = block_comodulation;
